"""Join the two validated low-fee post-event annual series."""
...
VARIANTS = ("baseline", "risk-on")
VARIANT_DIRS = {"baseline": "baseline", "risk-on": "risk_on"}
COMPARISON_COLUMNS = [
    "Year",
    "SPY Start",
    "SPY End",
...
def combine_variant_rows(
    ...
    for year in years:
        ...
        for variant, label in (("baseline", "Baseline"), ("risk-on", "Risk-On")):
...
def write_comparison(
    ...
    write_manifest(
        output,
        command="pre-earnings-post-event-low-fee-comparison",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Join the two validated low-fee post-earnings annual series.",
    )
