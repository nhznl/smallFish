...
def _human_report(result: SimulationResult) -> str:
    lines = [
        ...
        "Return fields are since inception from the original $50,000 strategy origin;",
    ...
    for arm, stats in result.summary.get("arms", {}).items():
        lines.extend([
            ...
            f"- Total return (since inception): {stats.get('total_return')}",
            f"- Passive SPY return (since inception, liquidation-marked): "
            f"{stats.get('benchmark_return')}",
            f"- Excess return (since inception): {stats.get('excess_return')}",
