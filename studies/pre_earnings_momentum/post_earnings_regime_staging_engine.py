"""Pure Study 6 weekly SPXL/SPY staging simulation.
...
the separate ETF-aware state and checkpoint contract required by Study 6.
"""
...
class RegimeStagingMark:
    ...
    benchmark_net_liquidation_value: float | None
...
def target_symbol(cfg: StudyConfig, regime: str) -> str:
    if cfg.staging_policy == legacy.STAGING_POLICY_SPY_ONLY:
    ...
    raise ValueError("Study 6 requires an ETF-staging policy")
...
def validate_required_etf_data(
    ...
    required = {cfg.defensive_staging_symbol, cfg.risk_on_staging_symbol}
...
def _schedule_cutoff(
    *,
    ...
    for ticker in tickers:
        if ticker in {cfg.benchmark_symbol, cfg.risk_on_staging_symbol}:
            continue
...
def run_regime_staging_simulation(
    *,
    ...
    if cfg.staging_policy == legacy.STAGING_POLICY_LEGACY_SPY:
        raise ValueError("legacy SPY configs cannot run through Study 6")
    ...
    if initial_checkpoint is not None:
        ...
        benchmark = copy.deepcopy(initial_checkpoint.benchmark)
    elif initial_state is not None:
        state = copy.deepcopy(initial_state)
        shadow = initial_state.copy_for_shadow()
        benchmark = {"shares": 0.0, "cash": cfg.starting_equity, "cost": 0.0}
    else:
    ...
    last_weekly_regime = REGIME_UNKNOWN
    first_execution = benchmark.get("shares", 0.0) > 0
    ...
    for index, session in enumerate(decision_sessions):
        if state.pending_target is not None and state.pending_target.execution_date == session:
            ...
            _fill_execution(
                cfg=cfg, session=session, state=state, shadow=shadow, target=target,
                stock_frames=stock_frames, asset_frames=asset_frames,
                sessions=all_sessions, spy=spy,
                orders=orders, trades=trades,
            )
            last_weekly_regime = target.regime
            state.pending_target = None
            if not first_execution:
                spy_open = _asset_price(asset_frames, cfg.benchmark_symbol, session, "open")
                shares = int(math.floor(
                    cfg.starting_equity / purchase_cash_per_share(spy_open, cfg)
                ))
                principal = shares * spy_open
        ...
        spy_close = _asset_price(asset_frames, cfg.benchmark_symbol, session, "close")
        benchmark_value = None
        benchmark_net = None
        if benchmark.get("shares", 0.0):
            benchmark_value = benchmark["cash"] + benchmark["shares"] * spy_close
            benchmark_net = benchmark_value - modeled_cost(
                benchmark["shares"] * spy_close, cfg,
        ...
        marks.append(RegimeStagingMark(
            ...
            total_equity=equity, net_liquidation_value=net_liquidation,
            benchmark_value=benchmark_value,
            benchmark_net_liquidation_value=benchmark_net,
            ...
            etf_shares=state.etf_shares, daily_market_regime=descriptive_regime,
            last_executable_weekly_regime=last_weekly_regime,
    ...
    checkpoint = RegimeStagingCheckpoint(
        ...
        shadow=copy.deepcopy(shadow), benchmark=copy.deepcopy(benchmark),
    )
    final = marks[-1]
    summary = {
        ...
        "benchmark_value": final.benchmark_value,
        "benchmark_net_liquidation_value": final.benchmark_net_liquidation_value,
...
def checkpoint_from_payload(
    ...
    *, expected_source_year: int | None = None,
    ...
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("unsupported Study 6 checkpoint schema")
    if payload.get("study_family") != "pre-earnings-weekly-regime-staging-v1":
        raise ValueError("checkpoint is not a Study 6 checkpoint")
    expected = {
    ...
    return RegimeStagingCheckpoint(
        ...
        benchmark={str(key): float(value) for key, value in payload["benchmark"].items()},
    )
