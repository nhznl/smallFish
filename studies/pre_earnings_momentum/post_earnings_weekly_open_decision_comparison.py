"""Validate and compare the two Study 5 exploratory annual chains."""
...
STUDY_ID = "pre-earnings-post-event-weekly-open-decision-v1"
COMMAND = "pre-earnings-post-event-weekly-open-decision-comparison"
...
def write_comparison(output: Path, rows: list[dict[str, str]], evidence: dict) -> None:
    ...
    validation_temporary.write_text(
        json.dumps(
            {**evidence, "comparison_sha256": sha256_file(output)},
            indent=2,
            sort_keys=True,
        ) + "\n",
        encoding="utf-8",
    ...
    write_manifest(
        ...
        extra={
            ...
            "evidence_label": "NO_VERDICT / EXPLORATORY",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Join the two Study 5 annual checkpoint chains.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.start_year != 2010 or args.end_year != 2025:
            raise ValueError("Study 5 comparison requires the complete 2010-2025 chain")
        ...
        evidence["evidence_label"] = "NO_VERDICT / EXPLORATORY"
