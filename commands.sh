...
case "$1" in
  ...
  rsi-supertrend-study)
    ...
    ;;
  market-regime-study)
    cd "$ROOT" && "$UTILITIES_PYTHON" -m studies.market_regime.experiment "${@:2}"
    ;;
  market-regime-compare)
    cd "$ROOT" && "$UTILITIES_PYTHON" -m studies.market_regime.comparison "${@:2}"
    ;;
  market-regime-holdout)
    cd "$ROOT" && "$UTILITIES_PYTHON" -m studies.market_regime.holdout "${@:2}"
    ;;
  ...
  pre-earnings-post-event-study)
    ...
    ;;
  pre-earnings-post-event-low-fee-study)
    cd "$ROOT" && "$UTILITIES_PYTHON" -m studies.pre_earnings_momentum.post_earnings_low_fee "${@:2}"
    ;;
  ...
  pre-earnings-post-event-weekly-batch-extension)
    ...
    ;;
  pre-earnings-post-event-weekly-open-decision)
    cd "$ROOT" && "$UTILITIES_PYTHON" -m studies.pre_earnings_momentum.post_earnings_weekly_open_decision "${@:2}"
    ;;
  ...
  pre-earnings-regime-staging-comparison)
    ...
    ;;
  pre-earnings-defensive-regime-staging)
    cd "$ROOT" && "$UTILITIES_PYTHON" -m studies.pre_earnings_momentum.post_earnings_defensive_regime_staging "${@:2}"
    ;;
  pre-earnings-defensive-regime-staging-comparison)
    cd "$ROOT" && "$UTILITIES_PYTHON" -m studies.pre_earnings_momentum.post_earnings_defensive_regime_staging_comparison "${@:2}"
    ;;
