...
Captured 2026-07-29. Data mode: `bootstrap-data` starter universe (97 ETFs plus
...
Option-Adjusted Basis. The connected set shows Holdings and the Symbol Ledger;
Option-Adjusted Basis is the same shared shell and is documented in
...
5. For connected brokerage frames, write synthetic ledger CSVs under a dedicated
   ...
   so the live checkout is untouched. Click the Options tab (and Holdings where
   needed) before capturing — those tabs are not URL routes of their own.
