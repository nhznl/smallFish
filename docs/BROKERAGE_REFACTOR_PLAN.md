...
| `d0285cc` | Delete the legacy options/ledger projection surface outright |

**Last verified baselines:** backend `stock-app/tests` 447 passing; Angular
`npm run test:ci` 69 passing; `npm run build` clean; `tools/check_docs.py` and
`tools/scan_secrets.py` clean; verified live on an isolated port-8001 server
over a synthetic-only data root — the four retired routes (`/options`,
`/options/activity`, `/retirement/options`,
`/brokerage-ledgers/{portfolio}/combined`) fall through to the Angular SPA
(`text/html`) instead of answering JSON, every retained resource
(`/api/brokerages/*`, `/options/activity/sync`, `/options/activity/manual`)
still answers correctly, and both brokerage pages render with no console
errors.
