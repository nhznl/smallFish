...
**Next action:** the group-only audit is done and its findings are in the
progress log. One decision is open for the owner: whether to retire the group
*write* path outright — the `legacy_groups` branch in both syncs, the five
now-unreachable mutators, and the `group_id` input on
`POST /options/activity/manual` — leaving `options_groups.csv` and
`options_group_members.csv` as read-only rollback artifacts that `snapshot` and
`risk_rows` keep serving to their frozen contracts. The writes are already off
by default; this would remove the ability to turn them back on. Do not remove
provider ingestion, parsers, canonical facts, or read-only rollback artifacts
merely because they contain legacy terminology.
